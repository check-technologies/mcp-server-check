...
async def test_list_contractor_payments_with_filters(mock_api, ctx):
    ...
    assert req.url.params.get_list("payroll") == ["prl_789"]


@pytest.mark.anyio
async def test_list_contractor_payments_payroll_multi_filter(mock_api, ctx):
    route = mock_api.get("/contractor_payments").mock(
        return_value=httpx.Response(
            200,
            json={"next": None, "previous": None, "results": []},
        )
    )
    await list_contractor_payments(ctx, payroll=["prl_001", "prl_002"])
    assert route.calls[0].request.url.params.get_list("payroll") == [
        "prl_001",
        "prl_002",
    ]
