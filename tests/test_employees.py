...
async def test_update_employee(mock_api, ctx):
    ...
    result = await update_employee(
        ctx, employee_id="emp_001", first_name="Janet", active=False
    )
    assert result["first_name"] == "Janet"
    req = mock_api.patch("/employees/emp_001").calls.last.request
    assert json.loads(req.content) == {"first_name": "Janet", "active": False}
